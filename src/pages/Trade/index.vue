<template>
  <div class="trade-container">
    ...
    <div class="sub clearFix">
      <!--下面router-link标签的点击事件哪怕添加.native也无效，不会触发点击事件，不知道为啥，因此只能改成a标签-->
      <!--<router-link class="subBtn" to="/pay" @click.native="submitOrder">提交订单</router-link>-->
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.getTradeAddressInfo();
      this.getTradeOrderInfo();
    },
    computed: {
      // ...mapState({
    ...
    methods: {
      ...
      //mock获取交易信息
      async getTradeAddressInfo() {
        try {
           //成功
          let response = await this.$API.reqAddressInfo();
          console.log("******获取用户地址信息response:{}", response)
      ...
      //mock获取订单信息
      async getTradeOrderInfo() {
        try {
          //成功
          let response = await this.$API.reqOrderInfo();
          console.log("******获取商品清单response:{}", response)
